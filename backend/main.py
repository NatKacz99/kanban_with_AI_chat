...
def create_column(payload: ColumnCreate):
    with get_connection() as conn:
        ...
        if position is None:
            row = conn.execute(
                "SELECT COALESCE(MAX(position), -1) as p FROM columns"
            ).fetchone()
            position = row["p"] + 1

        conn.execute(
            "INSERT INTO columns (id, board_id, title, position) VALUES (?, ?, ?, ?)",
            (col_id, "board-1", payload.title, position)
        )
        return build_board(conn)
...
def delete_column(column_id: str):
    with get_connection() as conn:
        ...
        res = conn.execute("DELETE FROM columns WHERE id = ?", (column_id,))
        if res.rowcount == 0:
            raise HTTPException(status_code=404, detail="Column not found")
        return build_board(conn)
...
def update_card(card_id: str, payload: CardUpdate):
    with get_connection() as conn:
        res = conn.execute(
            ...
            (payload.title, payload.details, card_id)
        )
        if res.rowcount == 0:
...
def delete_card(card_id: str):
    with get_connection() as conn:
        res = conn.execute("DELETE FROM cards WHERE id = ?", (card_id))
        if res.rowcount == 0:
            raise HTTPException(status_code=404, detail="Card not found")
        return build_board(conn)
